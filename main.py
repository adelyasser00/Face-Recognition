...
#index to sort the eigen vlaues and eigen vectors in decreasing order of eigen values
idx = np.argsort(eigenvalues)[::-1]
sorted_eigenvalues = eigenvectors[idx]
sorted_eigenvectors = eigenvectors[:,idx]

#sum to get the variance fraction to choose how many dimensiosn aka how many eigen vectors
cumulative_sum = np.cumsum(sorted_eigenvalues)

# alpha=[0.8,0.85,0.9,0.95]    loop on the array and mark accuracy
alpha = 0.95  # for example
...
#accuracy
print(f'Accuracy: {accuracy_score(testing_labels, predicted_labels)}')